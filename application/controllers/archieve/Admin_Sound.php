...
class Admin_Sound extends CI_Controller {
  ...
  public function index(){
    ...
    foreach($collections as $collection){
      $pageData['collections'][$collection['id']] = $collection;
    }
  ...
  public function Add(){
    ...
    if($_FILES['sound_file']['error'] == 0 && $_FILES['sound_artwork']['error'] == 0){
      $soundConfig['upload_path'] = "assets/site/sounds/";
      $soundConfig['allowed_types'] = 'mp3|wav|zip|rar';
      $soundConfig['allowed_types'] = $allowedType;
      $soundConfig['encrypt_name'] = true;
      $this->load->library("upload", $soundConfig);
      ...
      $response['responseMessage'] = $this->Common_Model->error('Error with Sound');
    }
  ...
  public function Update(){
    ...
    if($_FILES['sound_artwork']['error'] == 0){
      $config['upload_path'] = "assets/site/sounds/artworks/";
      $config['allowed_types'] = 'jpeg|jpg|png';
      $config['encrypt_name'] = true;
      $this->load->library("upload", $config);
      if ($this->upload->do_upload('sound_artwork')) {
        $sound_artwork = $this->upload->data("file_name");

        $update['sound_artwork'] = "assets/site/sounds/artworks/" .$sound_artwork;
        if(file_exists($update['sound_artwork_old'])) unlink($update['sound_artwork_old']);
      }
    }
    unset($update['sound_file_old']);
    unset($update['sound_artwork_old']);
