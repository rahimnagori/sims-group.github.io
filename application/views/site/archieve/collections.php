            ...
            <div class="fields d-block d-flex">
              <div class="select-wrap one-third">
                ...
                <select name="" id="category_id" class="form-control" placeholder="Keyword search">
                  ...
                  ?>
                      <option value="<?=$subCategory['id'];?>" <?=($currentCategoryId == $subCategory['id']) ? 'selected' : '';?> ><?=$subCategory['name'];?></option>
                  <?php
                    }
                  ?>
                </select>
      ...
      <div class="col-md-2">
        <aside class="aside_list">
          <ul class="list_view">
            <?php
              foreach($subCategories as $subCategory){
                // if($currentCategoryId == $subCategory['id']) continue;
                $activeClass = ($currentCategoryId == $subCategory['id']) ? "active" : '';
            ?>
                <li class="<?=$activeClass;?>"><a href="<?=$BASE_URL .'Collections/' .$subCategory['id'];?>"><?=$subCategory['name'];?></a></li>
            <?php
              }
            ?>
          </ul> 
...
<script>
  ...
  function search_category(e){
    ...
    window.location = base_url + 'Collections/' + category_id;
  }
