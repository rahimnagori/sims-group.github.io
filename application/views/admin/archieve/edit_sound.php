...
<div class="form-group">
  ...
  <select name="sound_collection_id" class="form-control" >
    ...
    ?>
        <option value="<?=$collection['id'];?>" <?=($soundDetails['sound_collection_id'] == $collection['id']) ? 'selected' : '';?> ><?=$collection['collection_name'];?></option>
    <?php
      }
    ?>
  </select>
...
<div class="form-group">
  ...
  <input type="file" accept="image/*" name="sound_artwork" onchange="preview_image(this, 'edit_preview_image');"  >
  <img id="edit_preview_image" width="100" src="<?=base_url($soundDetails['sound_artwork']);?>" >
  <input type="hidden" name="sound_artwork_old" value="<?=$soundDetails['sound_artwork'];?>" >
</div>
